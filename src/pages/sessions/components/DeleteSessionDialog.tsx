...
export const DeleteSessionDialog: React.FC<DeleteSessionDialogProps> = ({
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            toast(RenderSuccessToast("Session deleted successfully!"));
            onOpenChange(false);
        } catch (error) {
            toast(RenderFailedToast("Failed to delete session"));
        } finally {
            setLoading(false);
        }
    };
            ...
            <AlertDialogContent>
                ...
                <div className="py-4 space-y-3">
                    <div className="p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-xs text-gray-600">Date: {new Date(session.date).toLocaleDateString()}</p>
                    </div>
                    <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                        <p className="text-sm text-red-800">
                            <strong>Warning:</strong> This action cannot be undone. All attendance records for this session will also be deleted.
                        </p>
                    </div>
                </div>
                <AlertDialogFooter>
