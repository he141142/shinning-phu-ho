...
import {
  RegistrationCardInfoData,
  RegistrationCardInfoProps,
...
import { useUpdateStudent, useGetStudentDetail } from "@/hooks/students";
...
export default function StudentDetailPage() {
  ...
  // Parse student ID from router query
  const studentId = useMemo(() => {
    return id ? parseInt(id as string, 10) : 0;
  }, [id]);

  // Fetch data using React Query hook
  const { data: queryData, error, isLoading, refetch } = useGetStudentDetail(studentId);

  // Extract student data from the query response
  const data = useMemo(() => {
    if (!queryData?.GetStudentDetail) return undefined;
    return { GetStudentDetail: queryData.GetStudentDetail };
  }, [queryData]);
  ...
  const toStudentProps = useCallback(
    (data: { GetStudentDetail: any } | undefined): StudentProfileProps => {
      return {
        student: {
          ...
          emergency_contact_name:
            data?.GetStudentDetail.emergency_contact_name || "",
          emergency_contact_phone:
            data?.GetStudentDetail.emergency_contact_phone || "",
  ...
  const toClassesProps = useCallback(
    (data: { GetStudentDetail: any } | undefined): ClassesListProps => {
      return {
  ...
  const prepareRegistrationCardInfoProps = useCallback(
    (data: { GetStudentDetail: any } | undefined): RegistrationCardInfoData => {
      return {
  ...
  const handleFieldChange = useCallback(
    (field: keyof UpdateStudentInput, value: string) => {
  ...
  const isModalOpen = useMemo(
    () => isOpen && modalType === ModalType.joinClass,
    [isOpen, modalType]
  );
  ...
  if (error) {
    return <ErrorPage message={error.message || "Failed to load student details"} />;
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        <JoinClassModal
          ...
          registrationInfo={prepareRegistrationCardInfoProps(data)}
        />
        ...
        >
          ...
          <Tabs defaultValue="profile" className="px-6 pb-8">
            ...
            <TabsContent value="profile" className="mt-0">
              <StudentProfile
                student={toStudentProps(data).student}
                isEditing={isEditing}
            ...
            <TabsContent value="classes" className="mt-0">
              ...
              <ClassesList
                classes={toClassesProps(data).classes}
              />
            </TabsContent>
          </Tabs>
        </div>
